...
<body>
	<form id="form1">
		...
		<div class="form-group">
			<label for="picture">标题图片:</label> <input id=""picture""
				class="form-control-file" type="file" name="file">
		</div>
		<div class="form-group form-inline">
			<label for="channel">栏目</label> <select class="form-control"
				id="channel" name="channelId">
				<option>请选择</option>
			</select> <label for="category">分类</label> <select class="form-control"
				id="category" name="categoryId">
				<option>请选择</option>
			</select>
		</div>
		<textarea name="content1" cols="100" rows="8"
			style="width: 1027.22px; height: 200px; visibility: hidden;"><%=htmlspecialchars(htmlData)%></textarea>
		<br /> <input type="button" name="button" onclick="publish()"
			class="btn btn-info" value="发布" />
</body>
	...
	$(function() {
		...
		$("#channel").change(
				function() {
					...
					$("#category").empty();//先清空原有的分类
					$.get("/my/selectCategorysByChannelId", {
						channelId : channelId
					}, function(categorys) {
						//遍历所有的分类，动态追加option
